...
namespace Api.Controllers
{
    public class MenuController : BaseApiController
    {
        ...
        private readonly IAppLogger<Menu> _loger;

        public MenuController(IMenuService service, IAppLogger<Menu> loger)
        {
            _service = service;
            _loger = loger;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync([FromBody] Menu data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
            ...
            if (id == null)
            {
                _loger.LogError("Duplicate data name : " + data.Name);
        ...
        [HttpPut]
        public async Task<IActionResult> PutAsync([FromBody] Menu data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
        ...
        public async Task<IActionResult> IsExistDataWithKeyAsync([FromBody] ExistWithKeyModel model)
        {
            ...
            else if (typeof(Menu).HasProperty(model.KeyName.ToLower()) && typeof(Menu).HasProperty(model.FieldName.ToLower()))
            {
        ...
        [HttpGet("binding/select2/parent")]
        public async Task<IActionResult> BindingSelect2ParentAsync()
        {
            var data = await _service.BindingSelect2ParentAsync();
        ...
        [HttpGet("binding/select2/parent/{id}")]
        public async Task<IActionResult> BindingSelect2ParentAsync(Guid id)
        {
            var data = await _service.BindingSelect2ParentAsync(id);
