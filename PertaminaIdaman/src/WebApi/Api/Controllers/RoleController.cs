...
namespace Api.Controllers
{
    public class RoleController : BaseApiController
    {
        ...
        private readonly IAppLogger<Role> _loger;

        public RoleController(IRoleService service, IAppLogger<Role> loger)
        {
            _service = service;
            _loger = loger;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync([FromBody] Role data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
        ...
        [HttpPut]
        public async Task<IActionResult> PutAsync([FromBody] Role data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
        ...
        public async Task<IActionResult> IsExistDataWithKeyAsync([FromBody] ExistWithKeyModel model)
        {
            ...
            else if (typeof(Role).HasProperty(model.KeyName.ToLower()) && typeof(Role).HasProperty(model.FieldName.ToLower()))
            {
