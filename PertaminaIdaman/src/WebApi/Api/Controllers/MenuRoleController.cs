...
namespace Api.Controllers
{
    public class MenuRoleController : BaseApiController
    {
        ...
        private readonly IAppLogger<MenuRole> _loger;

        public MenuRoleController(IMenuRoleService service, IAppLogger<MenuRole> loger)
        {
            _service = service;
            _loger = loger;
        }
        ...
        [HttpGet("get/all/{fkRoleId}")]
        public async Task<IActionResult> GetAllAsync(Guid fkRoleId)
        {
            var data = await _service.GetAllAsync(fkRoleId);
            var result = new OK("Success get all data", data);
            return result.ReturnResponse();
        }
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync([FromBody] MenuRole data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
            ...
            if (id == null)
            {
                ...
                var error = new Forbidden("Duplicate data", new { data });
                return error.ReturnResponse();
            }
        ...
        [HttpPut]
        public async Task<IActionResult> PutAsync([FromBody] MenuRole data)
        {
            if (data == null)
            {
                _loger.LogError("Bad request parameter");
        ...
        public async Task<IActionResult> IsExistDataWithKeyAsync([FromBody] ExistWithKeyModel model)
        {
            ...
            else if (typeof(MenuRole).HasProperty(model.KeyName.ToLower()) && typeof(MenuRole).HasProperty(model.FieldName.ToLower()))
            {
